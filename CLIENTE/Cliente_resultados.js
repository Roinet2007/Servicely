...
// ... (código existente del sidebar y modales)

// Función para filtrar y mostrar resultados basados en el término de búsqueda
function displaySearchResults(searchTerm) {
    const resultsTitle = document.querySelector('.providers-title');
    const resultsSubtitle = document.querySelector('.providers-subtitle');
    
    if (resultsTitle) {
        resultsTitle.textContent = `Resultados para "${searchTerm}"`;
    }
    if (resultsSubtitle) {
        resultsSubtitle.textContent = `Encuentra el profesional perfecto para "${searchTerm}"`;
    }
    
    // Aquí puedes agregar lógica para filtrar los proveedores según el término de búsqueda
    // Por ejemplo:
    const providerCards = document.querySelectorAll('.provider-card');
    providerCards.forEach(card => {
        const providerName = card.querySelector('.provider-category').textContent.toLowerCase();
        const providerCategory = card.querySelector('.provider-category').textContent.toLowerCase();
        const providerSkills = Array.from(card.querySelectorAll('.skill-tag')).map(tag => tag.textContent.toLowerCase());
        
        if (providerName.includes(searchTerm.toLowerCase()) || 
            providerCategory.includes(searchTerm.toLowerCase()) ||
            providerSkills.some(skill => skill.includes(searchTerm.toLowerCase()))) {
...
// Cargar el término de búsqueda al cargar la página
document.addEventListener('DOMContentLoaded', function() {
    // Obtener el término de búsqueda de la URL o sessionStorage
    const urlParams = new URLSearchParams(window.location.search);
    let searchTerm = urlParams.get('search') || sessionStorage.getItem('searchTerm');
    ...
    // Configurar el evento de búsqueda en la página de resultados
    const searchButton = document.querySelector('.search-icon');
    if (searchButton) {
        searchButton.addEventListener('click', function() {
            const newSearchTerm = document.querySelector('.search-input').value.trim();
            if (newSearchTerm) {
                sessionStorage.setItem('searchTerm', newSearchTerm);
